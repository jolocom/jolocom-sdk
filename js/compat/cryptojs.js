...
exports.reencryptWithJolocomLib = exports.AESDecrypt = exports.CryptoJS = void 0;
exports.CryptoJS = {
    AES: {
        decrypt: AESDecrypt,
    },
...
exports.default = exports.CryptoJS;
 ...
 */
function OpenSSLFormatterParse(cipherTextBase64) {
    ...
    if (cipherText.readUInt32BE(0 * 4) == 0x53616c74 &&
 ...
 */
function OpenSSLKdfExecute(password, keySize, ivSize, salt) {
    // Generate random salt
    if (!salt) {
        salt = crypto_1.randomBytes(64 / 8);
    }
    // Derive key and IV
...
function AESDecrypt(cipherText, pass) {
    ...
    const { salt, ciphertext: cipherTextBuffer } = exports.CryptoJS.format.OpenSSL.parse(cipherText);
    // then the decryptor generates a key and IV using the default KDF (Key
    // Derivation Function) OpenSSLKdf.execute (cipher-core.js)
    //
    const { keySize, ivSize } = exports.CryptoJS.algo.AES;
    const { key, iv } = exports.CryptoJS.kdf.OpenSSL.execute(pass, keySize, ivSize, salt);
...
function reencryptWithJolocomLib(cipherTextBase64, password) {
    // NOTE: seed is encrypted as a hex representation, so we convert toString
    // (ascii) and then parse as hex
    const encodedSeed = exports.CryptoJS.AES.decrypt(cipherTextBase64, password).toString();
...
exports.reencryptWithJolocomLib = reencryptWithJolocomLib;
